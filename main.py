...
from data_utils import (
    Dataset_in_the_wild_eval, Dataset_in_the_wild_train, Dataset_PartialSpoof_dev, Dataset_RADAR, Dataset_train, genSpoof_list,
    Dataset_PartialSpoof_train,
    load_seglab, parse_ps_protocol, parse_radar_labels,
)
...
def _safe_num_workers(requested):
    """Cross-platform DataLoader worker count.
    - requested >= 0 : use as-is
    - requested == -1: auto (Windows=2, Linux/macOS up to 8)
    """
    if requested >= 0:
        return requested
    cpu_cnt = os.cpu_count() or 4
    if os.name == 'nt':          # Windows: spawn is heavy, keep modest
        return min(2, cpu_cnt)
    return min(8, cpu_cnt)        # Linux/macOS: fork is cheap, use more


def build_mixed_sampler(lengths, domain_probs):
    """Build a weighted sampler for concatenated datasets.
    lengths: list of per-domain dataset lengths in ConcatDataset order.
    domain_probs: desired sampling mass per domain, same order as lengths.
    """
    weights = []
    for n, p in zip(lengths, domain_probs):
        if n <= 0 or p <= 0:
            continue
        weights.extend([p / float(n)] * n)
    if not weights:
        return None
...
def evaluate_accuracy(dev_loader, model, device, debug_steps=0, epoch=0):
    """Validation loop aligned with mixed fine-tuning.
    dev_loader yields: (waveform, frame_labels, boundary_labels, utt_label, is_fine_grained)
    - L_sent is computed on all samples.
    - L_loc / L_bound / L_dia are computed only on fine-grained samples.
    """
    val_loss = 0.0
    val_loc = 0.0
    val_sent = 0.0
    ...
    criterion_ce_sent = nn.CrossEntropyLoss()
    ...
    with torch.no_grad():
        for i, (batch_x, batch_fl, batch_bl, batch_ul, batch_is_fg) in enumerate(dev_loader):
            ...
            batch_ul = batch_ul.to(device)
            fine_mask = batch_is_fg.to(device=device, dtype=torch.bool)
            ...
            with AMP_AUTocast('cuda'):
                ...
                if fine_mask.any():
                    logits_dia_fg = logits_dia[fine_mask]
                    batch_fl_fg = batch_fl[fine_mask]
                    p_bound_logits_fg = p_bound_logits[fine_mask]
                    batch_bl_fg = batch_bl[fine_mask]
                    h_prime_fg = h_prime[fine_mask]
                    Bf = logits_dia_fg.shape[0]

                    loss_loc = criterion_ce(
                        logits_dia_fg.reshape(Bf * T, C),
                        batch_fl_fg.reshape(Bf * T)
                    )
                    loss_bound = criterion_bound(p_bound_logits_fg, batch_bl_fg) if lam2 > 0 \
                        else torch.tensor(0.0, device=device)
                    loss_dia = criterion_p2sg(
                        h_prime_fg, batch_fl_fg,
                    ...
                    loss_dia = torch.tensor(0.0, device=device)

                loss = lam1 * loss_loc + lam_sent * loss_sent + lam2 * loss_bound + lam3 * loss_dia

            val_loss += loss.item() * batch_size
            val_loc += loss_loc.item() * batch_size
            val_sent += loss_sent.item() * batch_size
            val_bound += loss_bound.item() * batch_size
            val_dia += loss_dia.item() * batch_size
    ...
    val_bound /= num_total
    val_dia /= num_total
    print('Val loss: %.4f (loc=%.4f sent=%.4f bnd=%.4f dia=%.4f)' % (
        val_loss, val_loc, val_sent, val_bound, val_dia))
    return val_loss
...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    model.eval()
    # Open once outside the loop: 'w' clears any stale/partial file up front,
    # and keeping the handle open avoids repeated open/close I/O overhead.
...
def train_epoch(train_loader, model, optimizer, device, epoch, checkpoint_dir=None, debug_steps=0,
    ...
    """Training epoch with decoupled routing.
    train_loader yields: (waveform [B,66800], frame_labels [B,208], boundary_labels [B,208], utt_label [B], is_fine_grained [B])
    ...
    scaler = scaler if scaler is not None else make_grad_scaler('cuda')
    pbar = tqdm(train_loader, total=len(train_loader))
    for step, (batch_x, batch_fl, batch_bl, batch_ul, batch_is_fg) in enumerate(pbar):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', type=int, default=-1,
                        help='DataLoader workers. -1=auto (Win:2, Linux:8), 0=single-process')
    ...
    parser.add_argument('--mix_finetune', default=False,
                        type=lambda x: (str(x).lower() in ['true','yes','1']),
                        help='Enable mixed fine-tuning with PartialSpoof + ASV2019 LA + In-the-Wild')
    parser.add_argument('--asv_path', type=str, default='./LA',
                        help='Root path to ASVspoof2019 LA dataset')
    parser.add_argument('--itw_path', type=str, default='C:/release_in_the_wild',
                        help='Root path to In-the-Wild dataset')
    parser.add_argument('--radar_flac_dir', type=str, default='C:/RADAR2026-dev/flac',
                        help='Directory containing RADAR .flac files')
    parser.add_argument('--radar_label_file', type=str, default='C:/RADAR2026-dev/label_RADAR2026-dev.txt',
                        help='RADAR label file path')
    parser.add_argument('--partial_ratio', type=float, default=0.5,
                        help='Target sampling mass for PartialSpoof in mixed fine-tuning')
    parser.add_argument('--asv_ratio', type=float, default=0.3,
                        help='Target sampling mass for ASVspoof2019 LA in mixed fine-tuning')
    parser.add_argument('--itw_ratio', type=float, default=0.2,
                        help='Target sampling mass for In-the-Wild in mixed fine-tuning')
    parser.add_argument('--radar_ratio', type=float, default=0.4,
                        help='Target sampling mass for RADAR in mixed fine-tuning')
    ...
    resume_from_same_dir = False
    if args.resume:
        resume_dir = os.path.dirname(os.path.abspath(args.resume))
        current_dir = os.path.abspath(model_save_path)
        resume_from_same_dir = (resume_dir == current_dir)
    ...
    # [V3 PATCH] unified loader for plain-state and full-state checkpoints
    def _extract_model_state_for_load(ckpt_path):
    ...
    if args.track == 'In-the-Wild':
        ...
        model.load_state_dict(_extract_model_state_for_load(
    ...
    _nw = _safe_num_workers(args.num_workers)
    _pin = (_nw > 0)  # pin_memory helps when workers > 0
    print('DataLoader workers: {} (pin_memory={})'.format(_nw, _pin))

    radar_labels, radar_all_ids = parse_radar_labels(args.radar_label_file)
    rng = np.random.RandomState(args.seed)
    radar_all_ids = list(radar_all_ids)
    rng.shuffle(radar_all_ids)
    radar_split_idx = max(1, int(0.9 * len(radar_all_ids)))
    radar_train_ids = radar_all_ids[:radar_split_idx]
    radar_dev_ids = radar_all_ids[radar_split_idx:]
    if len(radar_dev_ids) == 0:
        radar_dev_ids = radar_train_ids[-1:]
        radar_train_ids = radar_train_ids[:-1]
    print('RADAR split: train={} dev={}'.format(len(radar_train_ids), len(radar_dev_ids)))

    if args.mix_finetune:
        asv_labels, asv_ids = genSpoof_list(
            dir_meta=os.path.join(args.asv_path, 'ASVspoof2019_LA_cm_protocols', 'ASVspoof2019.LA.cm.train.trn.txt'),
            is_train=True,
            is_eval=False,
        )
        asv_set = Dataset_train(
            args=args,
            list_IDs=asv_ids,
            labels=asv_labels,
            base_dir=os.path.join(args.asv_path, 'ASVspoof2019_LA_train'),
            algo=args.algo,
        )
        itw_set = Dataset_in_the_wild_train(base_dir=args.itw_path, split='train')
        radar_train_set = Dataset_RADAR(
            list_IDs=radar_train_ids,
            labels=radar_labels,
            flac_dir=args.radar_flac_dir,
            apply_augment=True,
        )
        mixed_train_set = ConcatDataset([train_set, asv_set, itw_set, radar_train_set])
        mixed_lengths = [len(train_set), len(asv_set), len(itw_set), len(radar_train_set)]
        sampler = build_mixed_sampler(
            mixed_lengths,
            [args.partial_ratio, args.asv_ratio, args.itw_ratio, args.radar_ratio],
        )
        print('Mixed fine-tuning enabled: Partial={} ASV={} ITW={} RADAR={}'.format(*mixed_lengths))
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size,
            num_workers=_nw, shuffle=True, drop_last=True,
            pin_memory=_pin, persistent_workers=(_nw > 0))
    del train_set

    val_batch_size = args.batch_size
    radar_dev_set = Dataset_RADAR(
        list_IDs=radar_dev_ids,
        labels=radar_labels,
        flac_dir=args.radar_flac_dir,
        apply_augment=False,
    )
    print('RADAR-only validation enabled: dev={} val_batch_size={}'.format(
        len(radar_dev_set), val_batch_size))
    ...
    # Cross-experiment fine-tuning should reset best tracking for the new output directory,
    # but we keep optimizer/scheduler resume behavior for convenience.
    if args.resume and not resume_from_same_dir:
        print('[V3 RESUME] New experiment directory detected; resetting best-loss tracking for fresh fine-tuning.')
        resume_bests = None
        resume_best_loss = None
    ...
    if args.average_model:
        ...
        if n_avg == 0:
            ...
            if os.path.exists(best_single) and os.path.getsize(best_single) > 1000:
                ...
                model.load_state_dict(_extract_model_state_for_load(best_single))
            elif checkpoints:
                latest = os.path.join(model_save_path, checkpoints[-1])
                print('Loading latest checkpoint: {}'.format(latest))
                model.load_state_dict(_extract_model_state_for_load(latest))
            else:
