...
class Dataset_PartialSpoof_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        is_fine_grained = torch.tensor(True, dtype=torch.bool)

        return waveform, frame_labels, boundary_labels, utt_label, is_fine_grained
        # shapes: [66800]  [208] long  [208] float32  scalar long  scalar bool
...
class Dataset_PartialSpoof_dev(Dataset):
    """PartialSpoof validation dataset returning the same 5-tuple as training."""
    def __init__(self, list_IDs, seglab, utt_labels, base_dir):
        self.list_IDs = list_IDs
        self.seglab = seglab
...
def parse_radar_labels(label_file):
    """Parse RADAR dev labels.
    Expected format per line: utt_id duration label segment_desc
    Uses parts[0] as utt_id and parts[2] as label.
    Returns: (labels_dict, ordered_id_list)
    """
    labels = {}
    ids = []
    with open(label_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if key not in ('bonafide', 'spoof'):
                continue
            labels[utt_id] = 0 if key == 'bonafide' else 1
            ids.append(utt_id)
    return labels, ids


class Dataset_RADAR(Dataset):
    """RADAR in-domain dataset with sentence-level supervision only."""
    def __init__(self, list_IDs, labels, flac_dir, apply_augment=True, cut=CUT):
        self.list_IDs = list_IDs
        self.labels = labels
    ...
    def __getitem__(self, index):
        utt_id = self.list_IDs[index]
        audio_path = os.path.join(self.flac_dir, utt_id + '.flac')
        X = _load_audio(audio_path, sr=SR)
        if self.apply_augment:
            X = _robustness_augment(X, sr=SR)
        X_pad = pad(X, self.cut)
        waveform = Tensor(X_pad)
        frame_labels = torch.zeros(NUM_FRAMES, dtype=torch.long)
        boundary_labels = torch.zeros(NUM_FRAMES, dtype=torch.float32)
        utt_label = torch.tensor(self.labels[utt_id], dtype=torch.long)
        is_fine_grained = torch.tensor(False, dtype=torch.bool)
        return waveform, frame_labels, boundary_labels, utt_label, is_fine_grained
...
class Dataset_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        boundary_labels = torch.zeros(NUM_FRAMES, dtype=torch.float32)
        is_fine_grained = torch.tensor(False, dtype=torch.bool)
        return x_inp, frame_labels, boundary_labels, torch.tensor(target, dtype=torch.long), is_fine_grained


class Dataset_in_the_wild_train(Dataset):
    """Sentence-level in-the-wild train dataset with explicit 0=real, 1=fake labels."""
    def __init__(self, base_dir, split='train', cut=CUT):
        self.base_dir = base_dir
        self.split = split
        self.cut = cut
        self.samples = []
        for subdir, label in [('real', 0), ('fake', 1)]:
            folder = os.path.join(base_dir, split, subdir)
            if not os.path.isdir(folder):
                continue
            for name in sorted(os.listdir(folder)):
                if name.lower().endswith('.wav'):
                    self.samples.append((os.path.join(folder, name), label))
